...
2. Optional system prep (Raspberry Pi):
   sudo apt-get update
...
def publish_sensor_data():
    ...
    try:
        ...
        if result.rc == mqtt.MQTT_ERR_SUCCESS:
            ...
            print(f"  - Vibration X: {data['vibration']['x']} m/s^2")
            print(f"  - Vibration Y: {data['vibration']['y']} m/s^2")
            print(f"  - Vibration Z: {data['vibration']['z']} m/s^2")
            print(f"  - Vibration magnitude: {data['vibration']['magnitude']} m/s^2")
            print(f"  - Temperature: {data['temperature']} degC")
